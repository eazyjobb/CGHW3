...
namespace model {
	...
	struct Vertex {
		...
		void addBoneData(GLuint boneID, GLfloat weight) {
			...
			for (int i = 0; i < NUM_BONES_PER_VEREX; ++i) if (weight > Weights[i]) {
				IDs[i] = boneID; Weights[i] = weight; break;
			}
			GLfloat sum = 0;
			for (int i = 0; i < NUM_BONES_PER_VEREX; ++i) sum += Weights[i];
			for (int i = 0; i < NUM_BONES_PER_VEREX; ++i) Weights[i] /= sum;
